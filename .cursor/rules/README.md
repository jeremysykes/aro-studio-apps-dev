...
3. Read `/docs/CORE_PUBLIC_API.md`
...
- Update the relevant file in `/docs` first (or add an ADR in `adr/` for significant arch/technical decisions, or a record in `decisions/` for other decisions). For when to use docs vs adr vs decisions, and where to put diagrams, see [docs/DOCUMENTATION.md](../../docs/DOCUMENTATION.md). Significant arch/technical decisions → `adr/`. Other product/process/design decisions → `decisions/`. Diagrams → `diagrams/`, referenced from the doc or ADR.
