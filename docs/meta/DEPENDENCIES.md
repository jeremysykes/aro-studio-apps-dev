...
- **dotenv** — Developer convenience: loads `ARO_ACTIVE_MODULE` (and other vars) from `apps/desktop/.env` at startup so developers can switch modules without setting the env var in the shell. No new IPC or config layer.
