...
- **@aro/module-hello-world** — Standalone active module (default). Desktop loads it after Core init, calls its `init(core)` to register jobs, and renders its UI in the main content area.
...
- **zod** (inspect module only) — Runtime schema validation at module boundaries: scan input, export input, Figma API responses, and persisted report artifacts. Reason: external data (user config, API JSON, artifact JSON) must be validated before use; hand-written checks are brittle and verbose. ~13KB gzipped; already used by Core. Replaces: manual type guards and unchecked `as` casts on external data.
