...
function hasAtLeastOneSource(config: ScanConfig): boolean {
  ...
  const hasStorybook = config.storybookUrl.trim().length > 0 || config.storybookPath.trim().length > 0;
...
export default function Inspect() {
  ...
  const [runs, setRuns] = useState<Array<{ id: string; status: string; startedAt: number }>>([]);
  ...
  const [reportTab, setReportTab] = useState<'health' | 'tokens' | 'components'>('health');
  ...
  useEffect(() => {
    loadWorkspace();
    const unsub = window.aro.workspace.onChanged((data: { path: string } | null) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setLogs(logList);
        unsub = await window.aro.logs.subscribe(selectedRunId, (entry: { id: string; level: string; message: string }) => {
    ...
    <main className="min-w-[900px] min-h-screen p-6 font-sans" role="main">
      ...
      {!workspacePath ? (
        <Card className="mb-4">
          ...
          <CardContent>
            <p className="mb-2 text-muted-foreground">Select a workspace to configure sources and run Inspect.</p>
        ...
        <>
          ...
          {error && (
            <div role="alert" className="rounded-lg border border-red-200 bg-red-50 dark:bg-red-950/30 p-4 text-red-800 dark:text-red-200 mb-4">
              {error}
            </div>
          )}

          <nav className="flex gap-2 mb-4" aria-label="Inspect views">
            <Button
              type="button"
          ...
          {view === 'setup' && (
            <section aria-labelledby="setup-heading">
              ...
              <Card className="mb-4">
                ...
                <CardContent className="space-y-2">
                  <label className="block text-sm font-medium">
                    File key(s) <span className="text-muted-foreground">(comma-separated)</span>
                  </label>
                  <input
                    type="text"
                    className="w-full rounded border px-3 py-2 text-sm"
                    value={config.figmaFileKeys}
                    onChange={(e) => setConfig((c) => ({ ...c, figmaFileKeys: e.target.value }))}
                  ...
                  <input
                    ...
                    onChange={(e) => setConfig((c) => ({ ...c, figmaPat: e.target.value }))}
                    placeholder="figd_…"
              ...
              <Card className="mb-4">
                ...
                <CardContent>
                  <label className="block text-sm font-medium mb-1">
                    Path(s) to token files <span className="text-muted-foreground">(one per line or comma-separated)</span>
                  </label>
                  <textarea
                    className="w-full rounded border px-3 py-2 text-sm min-h-[80px]"
                    value={config.codePaths}
                    onChange={(e) => setConfig((c) => ({ ...c, codePaths: e.target.value }))}
              ...
              <Card className="mb-4">
                ...
                <CardContent className="space-y-2">
                  ...
                  <input
                    ...
                    onChange={(e) => setConfig((c) => ({ ...c, storybookPath: e.target.value }))}
              ...
              {!hasAtLeastOneSource(config) && (
                <p className="mt-2 text-sm text-muted-foreground">
                  Configure at least one source (Figma, Code tokens, or Storybook) to enable Run Inspect.
                </p>
              )}
            </section>
          )}
            ...
            <section aria-labelledby="run-heading">
                  ...
                  <ul className="list-none space-y-1">
                    {runs.map((run) => (
                      <li key={run.id}>
                        ...
                        >
                          {run.id.slice(0, 8)} — {run.status} — {new Date(run.startedAt).toLocaleString()}
                        </Button>
                      </li>
                    ))}
                  </ul>
              ...
              {selectedRunId && (
                <Card className="mt-4">
                  ...
                  <CardContent>
                    <ul className="list-none space-y-1 font-mono text-sm" role="log" aria-live="polite">
                      {logs.map((entry) => (
                        <li key={entry.id}>
          ...
          {view === 'report' && (
            <section aria-labelledby="report-heading">
              ...
              <Card className="mb-4">
                ...
                <CardContent>
                  <ul className="list-none space-y-1">
                    {runs.filter((r) => r.status === 'success').map((run) => (
                      <li key={run.id}>
                        <Button
                          type="button"
                          variant={selectedRunId === run.id ? 'default' : 'ghost'}
              ...
              {report && (
                <>
                  <div className="flex gap-2 mb-4" role="tablist" aria-label="Report tabs">
                    <Button
                      type="button"
                    ...
                    <Card>
                      ...
                      <CardContent>
                        ...
                        <ul className="list-disc pl-6">
                          <li>Critical: {report.summary.findingsBySeverity?.critical ?? 0}</li>
                        ...
                        <div className="mt-4 flex gap-2">
                          ...
                          <Button type="button" variant="outline" onClick={() => handleExport('markdown')}>
                            Export Markdown
                          </Button>
                        </div>
                      </CardContent>
                    ...
                    <Card>
                      ...
                      <CardContent className="overflow-x-auto">
                        <table className="w-full text-sm border-collapse">
                          <thead>
                            <tr>
                              <th scope="col" className="text-left border p-2">Name</th>
                              <th scope="col" className="text-left border p-2">Value</th>
                              <th scope="col" className="text-left border p-2">Type</th>
                              <th scope="col" className="text-left border p-2">Source</th>
                            </tr>
                          </thead>
                          <tbody>
                            {report.tokens.map((t) => (
                      ...
                      <CardContent className="overflow-x-auto">
                        <table className="w-full text-sm border-collapse">
                          <thead>
                            <tr>
                              <th scope="col" className="text-left border p-2">Name</th>
                              <th scope="col" className="text-left border p-2">Surfaces</th>
                              <th scope="col" className="text-left border p-2">Coverage</th>
                              <th scope="col" className="text-left border p-2">Orphan</th>
                            </tr>
                          </thead>
                          <tbody>
